...
class DExplorer:
    ...
    def select_box(self, predict, size, dims=208, stride=16):
        ...
        h, w, c = affines.shape
        affines = affines.reshape(h, w, 2, 3).numpy() #预测得到的仿射矩阵
        ...
        for i in range(h):
            for j in range(w):
                if probs[i, j, 1] > config.confidence_threshold:
                    affine = affines[i, j]
                    pts = affine @ unit    #矩阵乘法 得到预测的8个点
                    # print(affine)
                    # print(affine) #相对网格中心点的偏移坐标
                    pts *= scale    #放大到网格坐标
                    pts += numpy.array([[j + 0.5], [i + 0.5]]) #加上网格中心坐标
                    pts *= stride     #放大到整个图片的坐标
                    # print(pts)
                    pts /= wh     #放缩到相对图片的归一化坐标
                    # exit()
        ...
        labels = []
        # exit()
        '''非极大值抑制'''#其他lable和第一个lable重叠度较高的话就不要了
        for pts_c, prob_c in candidates:
            tl_c = pts_c.min(axis=1) # top left 坐标
            # print('tl_c:',tl_c)
            # exit()
            br_c = pts_c.max(axis=1)  #bottom right 坐标
            overlap = False
